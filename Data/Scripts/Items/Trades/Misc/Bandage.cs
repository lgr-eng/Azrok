...
namespace Server.Items
{
        ...
        private class InternalTarget : Target
        {
            ...
            protected override void OnTarget(Mobile from, object targeted)
            {
                ...
                if (targeted is Mobile)
                {
                    ...
                    targeted is HenchmanFighterItem
                    && from.Skills[SkillName.Anatomy].Value >= 80
                ...
                )
                {
                    ...
                    targeted is HenchmanWizardItem
                    && from.Skills[SkillName.Anatomy].Value >= 80
                    && from.Skills[SkillName.Healing].Value >= 80
                )
                {
                    ...
                    targeted is HenchmanArcherItem
                    && from.Skills[SkillName.Anatomy].Value >= 80
                    && from.Skills[SkillName.Healing].Value >= 80
                )
                {
                    ...
                    targeted is HenchmanMonsterItem
                    && from.Skills[SkillName.Anatomy].Value >= 80
                    && from.Skills[SkillName.Healing].Value >= 80
                )
                {
    ...
    public class BandageContext
    {
        ...
        private static Dictionary<Mobile, BandageContext> m_Table =
            new Dictionary<Mobile, BandageContext>();
        ...
        public void EndHeal()
        {
            ...
            else if (!m_Patient.Alive || (petPatient != null && petPatient.IsDeadPet))
            {
                ...
                if (
                    ...
                    || (
                        Core.SE
                        && petPatient is Factions.FactionWarHorse
                ...
                ) //TODO: Dbl check doesn't check for faction of the horse here?
                {
                    if (
                        m_Patient.Map == null
                        || !m_Patient.Map.CanFit(m_Patient.Location, 16, false, false)
                    )
                    {
            ...
            else if (m_Patient.Poisoned)
            {
                ...
                double anatomy = m_Healer.Skills[secondarySkill].Value;
                double chance =
                    ((healing - 30.0) / 50.0) - (m_Patient.Poison.Level * 0.1) - (m_Slips * 0.02);
        ...
        public static BandageContext BeginHeal(Mobile healer, Mobile patient)
        {
            ...
            else if (patient is BaseCreature && ((BaseCreature)patient).IsAnimatedDead)
            {
                ...
                && !BleedAttack.IsBleeding(patient)
                && !isDeadPet
            )
            {
                ...
                !patient.Alive
                && (patient.Map == null || !patient.Map.CanFit(patient.Location, 16, false, false))
            )
            {
