    ...
    class GetPlayerInfo
    {
            ...
            else if (m.SkillsTotal > 0)
            {
                ...
                if (highest != null)
                {
                    ...
                    else
                    {
                        ...
                        skillTitle = Skill.CharacterTitle(
                            skillTitle,
                            m.Female,
                            m.Karma,
                            m.Skills[SkillName.Knightship].Value,
                            m.Skills[SkillName.Seafaring].Value,
                            m.Skills[SkillName.Magery].Base,
                            m.Skills[SkillName.Necromancy].Base,
                            m.Skills[SkillName.Healing].Base,
                            m.Skills[SkillName.Spiritualism].Base,
                            isBarbaric,
                            isOriental,
                            isMonk(m),
                            isSyth(m, false),
        ...
        public static bool isJester(Mobile from)
        {
            ...
            if (from is PlayerMobile && from != null && from.Backpack != null)
            {
                ...
                if (
                    from.Skills[SkillName.Begging].Value > 10
                    || from.Skills[SkillName.Psychology].Value > 10
                )
                {
                    points++;
                }
        ...
        private static Skill GetHighestSkill(Mobile m)
        {
            ...
            for (int i = 0; i < m.Skills.Length; ++i)
            {
                ...
                if (highest == null || check.Value > highest.Value)
                    ...
                    highest != null
                    && highest.Lock != SkillLock.Up
                    && check.Lock == SkillLock.Up
                    && check.Value == highest.Value
                )
                    highest = check;
            }
        ...
        public static string GetTodaysDate()
        {
            ...
            string sDateString =
                sMonthName + " " + sDay + ", " + sYear + " at " + sHour + ":" + sMinute;
        ...
        public static bool EvilPlayer(Mobile m)
        {
            ...
            if (m is PlayerMobile)
            {
                ...
                if (
                    m.Skills[SkillName.Psychology].Base >= 50.0
                    && m.Skills[SkillName.Swords].Base >= 50.0
                    && m.Karma <= -5000
                    && Server.Misc.GetPlayerInfo.isSyth(m, false)
                ) // SYTH
                    return true;
        ...
        public static string GetWantedStatus(Mobile m)
        {
            ...
            if (m is PlayerMobile)
            {
                ...
                if (
                    m is PlayerMobile
                    && (m.Karma < 2500 || m.Fame < 2500)
                    && Server.Items.BaseRace.IsEvil(m)
                )
                {
                    warning =
                        warning
                        + "You are considered by most to be a vile creature and not welcome in many settlements. ";
                }
                if (
                    m is PlayerMobile
                    && m.Karma <= -5000
                    && m.Skills[SkillName.Knightship].Base >= 50
                )
                {
                    warning =
                        warning + "You are a death knight, which is feared amongst the land. ";
                }
                if (
                    m is PlayerMobile
                    && m.Karma <= -5000
                    && m.Skills[SkillName.Psychology].Base >= 50
                    && Server.Misc.GetPlayerInfo.isSyth(m, false)
                )
                {
                ...
                if (DisguiseTimers.IsDisguised(m) && warning != "")
                    warning =
                        warning
                        + "You could probably sneak into settlements, however, since you will not be recognized.";
    ...
    public class StatsGump : Gump
    {
        ...
        public static void Initialize()
        {
            CommandSystem.Register(
                "status",
                AccessLevel.Player,
                new CommandEventHandler(MyStats_OnCommand)
            );
        }
        ...
        public StatsGump(Mobile m, Mobile from, int origin)
            : base(50, 50)
        {
            m_Origin = origin;
            ...
            if (
                ...
                && Server.Misc.MyServerSettings.SpellDamageIncreaseVsMonsters() > 0
            )
            {
                SDICap = Server.Misc.MyServerSettings.SpellDamageIncreaseVsMonsters();
            }
            ...
            int LRC =
                AosAttributes.GetValue(from, AosAttribute.LowerRegCost) > LRCCap
                    ? LRCCap
                    : AosAttributes.GetValue(from, AosAttribute.LowerRegCost);
            int LMC =
                AosAttributes.GetValue(from, AosAttribute.LowerManaCost) > LMCCap
                    ? LMCCap
            ...
            TimeSpan SwingSpeed =
                (from.Weapon as BaseWeapon).GetDelay(from) > TimeSpan.FromSeconds(SwingSpeedCap)
                    ...
                    : (from.Weapon as BaseWeapon).GetDelay(from);
            int HCI =
                AosAttributes.GetValue(from, AosAttribute.AttackChance) > HCICap
                    ? HCICap
                    : AosAttributes.GetValue(from, AosAttribute.AttackChance);
            int DCI =
                AosAttributes.GetValue(from, AosAttribute.DefendChance) > DCICap
                    ? DCICap
                    : AosAttributes.GetValue(from, AosAttribute.DefendChance);
            int FC =
                AosAttributes.GetValue(from, AosAttribute.CastSpeed) > FCCap
                    ? FCCap
                    : AosAttributes.GetValue(from, AosAttribute.CastSpeed);
            int FCR =
                AosAttributes.GetValue(from, AosAttribute.CastRecovery) > FCRCap
                    ? FCRCap
                    : AosAttributes.GetValue(from, AosAttribute.CastRecovery);
            int DamageIncrease =
                AosAttributes.GetValue(from, AosAttribute.WeaponDamage) > DamageIncreaseCap
                    ? DamageIncreaseCap
                    : AosAttributes.GetValue(from, AosAttribute.WeaponDamage);
            int SDI =
                AosAttributes.GetValue(from, AosAttribute.SpellDamage) > SDICap
                    ? SDICap
                    : AosAttributes.GetValue(from, AosAttribute.SpellDamage);
            int ReflectDamage =
                AosAttributes.GetValue(from, AosAttribute.ReflectPhysical) > ReflectDamageCap
                    ? ReflectDamageCap
                    : AosAttributes.GetValue(from, AosAttribute.ReflectPhysical);
            int SSI =
                AosAttributes.GetValue(from, AosAttribute.WeaponSpeed) > SSICap
                    ? SSICap
                    : AosAttributes.GetValue(from, AosAttribute.WeaponSpeed);
            ...
            AddHtml(
                ...
                400,
                20,
                @"<BODY><BASEFONT Color=" + color + ">" + name.ToUpper() + "</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
            ...
            AddHtml(
                ...
                100,
                20,
                @"<BODY><BASEFONT Color=" + warnColor + ">" + warnMsg + "</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
            ...
            colB =
                colB
                + ""
                + String.Format(" {0} + {1}", from.RawStr, from.Str - from.RawStr)
                + "<BR><BR>";
            colB =
                colB
                + ""
                + String.Format(" {0} + {1}", from.RawDex, from.Dex - from.RawDex)
                + "<BR><BR>";
            colB =
                colB
                + ""
                + String.Format(" {0} + {1}", from.RawInt, from.Int - from.RawInt)
                + "<BR><BR>";
            ...
            AddHtml(
                ...
                200,
                370,
                @"<BODY><BASEFONT Color=" + color + ">" + colA + "</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
            AddHtml(
            ...
            colD =
                ...
                + String.Format(
                    " {0} + {1}",
                    from.Hits - AosAttributes.GetValue(from, AosAttribute.BonusHits),
                    AosAttributes.GetValue(from, AosAttribute.BonusHits)
                )
                + "<BR><BR>";
            colD =
                colD
                ...
                + String.Format(
                    " {0} + {1}",
                    from.Stam - AosAttributes.GetValue(from, AosAttribute.BonusStam),
                    AosAttributes.GetValue(from, AosAttribute.BonusStam)
                )
                + "<BR><BR>";
            colD =
                colD
                ...
                + String.Format(
                    " {0} + {1}",
                    from.Mana - AosAttributes.GetValue(from, AosAttribute.BonusMana),
                    AosAttributes.GetValue(from, AosAttribute.BonusMana)
                )
                + "<BR><BR>";
            colD =
                colD
                + ""
                + String.Format(" {0}", AosAttributes.GetValue(from, AosAttribute.RegenHits))
                + "<BR><BR>";
            colD =
                colD
                + ""
                + String.Format(" {0}", AosAttributes.GetValue(from, AosAttribute.RegenStam))
                + "<BR><BR>";
            colD =
                colD
                + ""
                + String.Format(" {0}", AosAttributes.GetValue(from, AosAttribute.RegenMana))
                + "<BR><BR>";
            ...
            AddHtml(
                ...
                150,
                380,
                @"<BODY><BASEFONT Color=" + color + ">" + colC + "</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
            AddHtml(
            ...
            colF =
                colF
                + ""
                + String.Format(" {0}s", new DateTime(SwingSpeed.Ticks).ToString("s.ff"))
                + "<BR><BR>";
            ...
            colF =
                ...
                + String.Format(
                    " {0:0.0}s",
                    new DateTime(TimeSpan.FromSeconds(BandageSpeed).Ticks).ToString("s.ff")
                )
                + "<BR><BR>";
            ...
            AddHtml(
                ...
                150,
                380,
                @"<BODY><BASEFONT Color=" + color + ">" + colE + "</BASEFONT></BODY>",
                (bool)false,
                (bool)false
            );
            AddHtml(
