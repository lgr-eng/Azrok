...
namespace Server.Items
{
    ...
    public class Container : Item
    {
        ...
        {
            get { return ContainerData.Bounds; }
        }
        public virtual int DefaultGumpID
        {
            get { return ContainerData.GumpID; }
        }
        public virtual int DefaultDropSound
        {
            get { return ContainerData.DropSound; }
        }

        public virtual int DefaultMaxItems
        {
            get { return m_GlobalMaxItems; }
        }
        public virtual int DefaultMaxWeight
        {
            get { return m_GlobalMaxWeight; }
        }
        ...
        public virtual bool IsDecoContainer
        {
            get
            {
                return !Movable && !IsLockedDown && !IsSecure && Parent == null && !m_LiftOverride;
            }
        }
        ...
        )
        {
            if (m.AccessLevel < AccessLevel.GameMaster)
            {
                ...
                if (
                    ...
                    && maxItems != 0
                    && (
                        this.TotalItems + plusItems + item.TotalItems + (item.IsVirtualItem ? 0 : 1)
                    ) > maxItems
                )
                {
                    if (message)
                ...
                else
                {
                    ...
                    if (
                        maxWeight != 0
                        && (this.TotalWeight + plusWeight + item.TotalWeight + item.PileWeight)
                            > maxWeight
                    )
                    {
                        if (message)
            ...
            while (parent != null)
            {
                if (parent is Container)
                    return ((Container)parent).CheckHold(
                        m,
                        item,
                        message,
                        checkItems,
        ...
        public bool ConsumeTotalGrouped(
            Type type,
            int amount,
            bool recurse,
            OnItemConsumed callback,
        ...
        private static void RecurseFindItemsByType(
            Item current,
            Type type,
            bool recurse,
            List<Item> list
        ...
        public List<T> FindItemsByType<T>()
            where T : Item
        {
            return FindItemsByType<T>(true, null);
        }

        public List<T> FindItemsByType<T>(bool recurse)
            where T : Item
        {
            return FindItemsByType<T>(recurse, null);
        }

        public List<T> FindItemsByType<T>(Predicate<T> predicate)
            where T : Item
        {
            return FindItemsByType<T>(true, predicate);
        }

        public List<T> FindItemsByType<T>(bool recurse, Predicate<T> predicate)
            where T : Item
        {
            if (m_FindItemsList.Count > 0)
                m_FindItemsList.Clear();
        ...
        private static void RecurseFindItemsByType<T>(
            Item current,
            bool recurse,
            List<T> list,
        ...
        public T FindItemByType<T>(Predicate<T> predicate)
            where T : Item
        {
            return FindItemByType<T>(true, predicate);
        }

        public T FindItemByType<T>(bool recurse)
            where T : Item
        {
            return FindItemByType<T>(recurse, null);
        }

        public T FindItemByType<T>(bool recurse, Predicate<T> predicate)
            where T : Item
        {
            return RecurseFindItemByType<T>(this, recurse, predicate);
        }

        private static T RecurseFindItemByType<T>(
            Item current,
            bool recurse,
            Predicate<T> predicate
        )
    ...
    public class ContainerData
    {
        static ContainerData()
        {
            ...
            string path = Path.Combine(Core.BaseDirectory, "Data/System/CFG/containers.cfg");
                        ...
                        if (split.Length >= 3)
                        {
                            ...
                            if (split.Length >= 4)
                            {
                                ...
                                for (int i = 0; i < aIDs.Length; i++)
                                {
                                    ...
                                    {
                                        Console.WriteLine(
                                            @"Warning: double ItemID entry in containers.cfg"
                                        );
                                    }
                                    else
                                    {
